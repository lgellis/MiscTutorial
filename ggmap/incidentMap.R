...
ggmap(get_googlemap(center = c(lon = -122.335167, lat = 47.608013),
                    ...
                    maptype ='terrain',
                    color = 'color')) +
  geom_point(aes(x = Longitude, y = Latitude,  colour = Initial.Type.Group), data = incidents, size = 0.5) + 
...
ggmap(get_googlemap(center = c(lon = -122.335167, lat = 47.608013),
                    ...
                    maptype ='terrain',
                    color = 'color')) +
  geom_point(aes(x = Longitude, y = Latitude), colour = col1, data = incidents, alpha=0.05, size = 0.5) + 
...
gc <- geocode('seattle, washington')
center <- as.numeric(gc)
ggmap(get_googlemap(center = center,
                    zoom = 11, scale = 2,
                    maptype ='terrain',
                    color = 'color'))  +
  stat_density2d(
...
ggmap(get_googlemap(center = center,
                    ...
                    maptype ='terrain',
                    color = 'color')) +
  geom_point(aes(x = Longitude, y = Latitude,  colour = Event.Clearance.Group), data = i2Sub, alpha=0.1, size = 2) + 
  theme(legend.position="bottom") +
